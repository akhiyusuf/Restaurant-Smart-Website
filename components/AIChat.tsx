...
const AIChat: React.FC<AIChatProps> = ({ 
  isOpen, onClose, onOpenCheckout, addToCart, removeFromCart, cart = [], 
  ...
  // Handle incoming suggested messages from the app (e.g. from Pairing buttons)
  useEffect(() => {
  ...
  const handleResponse = async (response: GenerateContentResponse | null) => {
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      for (const fc of functionCalls) {
        ...
        // Gemini SDK expects { functionResponse: { name, response, id? } }
        toolResponses.push({
          functionResponse: {
            name: fc.name,
  ...
  const handleSend = async (msgText: string = input) => {
    ...
    const response = await sendMessageToGemini(msgText, cart);
